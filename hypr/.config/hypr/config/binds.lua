...
-- Only dispatch to a monitor if it's actually connected, otherwise Hyprland
-- throws "Invalid monitor / monitor doesn't exist" (e.g. when undocked)
local function focusMonitor(mon)
	return function()
		if hl.get_monitor(mon) then
			hl.dispatch(hl.dsp.focus({ monitor = mon }))
		end
	end
end

local function moveToMonitor(mon)
	return function()
		if hl.get_monitor(mon) then
			hl.dispatch(hl.dsp.window.move({ monitor = mon }))
		end
	end
end
...
hl.bind(mainMod .. " + SHIFT + 1", moveToMonitor(MONITOR1))
hl.bind(mainMod .. " + SHIFT + 2", moveToMonitor(MONITOR2))
...
-- Focus on monitors
hl.bind(mainMod .. " + 1", focusMonitor(MONITOR1))
hl.bind(mainMod .. " + 2", focusMonitor(MONITOR2))
